...
## What we coin
- **AGM = Aitía · Gut · Metron** — the afferent-only WHY-layer self-model. In-paper canonical name:
  *"the Aitía–Gut–Metron (AGM) afferent self-model"* (disambiguates from the crowded "AGM" abbreviation:
  annual-general-meeting, AGM battery). Owned via the paper (DOI) + repo + guide + consistent use. Formal
  trademark = a later business step (as with Pallium's TM class-check).
- **The umbrella (LOCKED 2026-07-19): VGM — The Vektorgeist Method.** The named methodology for our overall
  approach (local-first · model-free-where-you-can · "model proposes / oracle disposes" · afferent self-modeling ·
  sovereign). Exactly parallel to Jake's "ICM." **AGM is VGM's flagship pillar.** (Acronym "VGM" collides with
  non-AI meanings [vector-graphics-metafile, video-game-music] — disambiguated by the full name + the VG brand,
  same as ICM. Formal TM = later business step.)

## The paper set
- **P1 — AGM (flagship coinage).** The afferent self-model. Draft exists → `Projects/agm-paper/agm.md`. Polish +
  references → Zenodo DOI + GitHub repo + guide. This is our "ICM."
...
- **Pipeline (not a one-off):** a `vektorgeist.com/research` page + a GitHub index repo listing every coined term +
...
- **vektorgeist.com/research** page (links papers + DOIs) + a **Substack/blog** writeup per paper — the funnel,
  tying into the VG Lyceum course + Pallium + the products.

## Sequencing
1. **Collision-check** AGM + the 3 umbrella-name candidates (web / Google Scholar / trademark prior-art) → lock names.
2. **Finalize P1 (AGM)** — polish draft, byline Alexander Parnell + Vektorgeist, real references, structural figure →
   Zenodo DOI + GitHub repo + guide.
3. **P2 (Pallium systems)** from the annex, design-level + grant-safe → Zenodo.
4. **P3 (program overview)** → Zenodo; build the `/research` page + index repo.
5. **arXiv** endorsement + Substack/HN rollout.

## Novelty positioning (from the 2026-07-19 collision/novelty check — related work for P1)
- The field is all **self-CORRECTION** (agent judges itself and CHANGES behavior — Reflexion, Process Reward
  Models, Meta self-correcting hyperagents, drift-resistance plugins) or **external observability** (Langfuse,
